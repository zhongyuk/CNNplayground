...
def compute_accuracy(pred_tensor, label_tensor):
    '''A func to compute accuracy given TF tensor as inputs'''
    correct_pred = tf.equal(tf.argmax(pred_tensor,1), tf.argmax(label_tensor,1))
...
def train_cnn(graph, model, tf_data, cnn_shapes, hyperparams, epoches, *args):
    ...
    with graph.as_default():
        ...
        tf.scalar_summary('training_loss', train_loss)
        ...
        with tf.name_scope('training_accuracy'):
            train_accuracy = compute_accuracy(train_prediction, tf_train_labels)
        ...
        tf.scalar_summary('validation_loss', valid_loss)
        valid_prediction = tf.nn.softmax(valid_logits)
        with tf.name_scope('validation_accuracy'):
            valid_accuracy = compute_accuracy(valid_prediction, tf_valid_labels)
        tf.scalar_summary('validation_accuracy', valid_accuracy)
        # Test Computation
        test_prediction = tf.nn.softmax(model(tf_test_dataset, scopes, False, batch_norm, keep_prob))
        test_accuracy = compute_accuracy(test_prediction, tf_test_labels)
  ...
  # Train Convnet
    ...
    with tf.Session(graph=graph) as session:
        train_writer = tf.train.SummaryWriter('logs/train', graph=session.graph)
        valid_writer = tf.train.SummaryWriter('logs/valid', graph=session.graph)
        ...
        for epoch in range(epoches):
            ...
            # Run session...
            lr, _, tl, tacc, train_summaries = session.run([learning_rate, optimizer, train_loss, train_accuracy, merge_summary_op], \
                                                        feed_dict=feed_dict)
            ...
            vl, vacc, valid_summaries = session.run([valid_loss, valid_accuracy, merge_summary_op])
            valid_writer.add_summary(valid_summaries, epoch)
            valid_losses[epoch] = vl
            valid_acc[epoch] = vacc
            print('Epoch: %d:\tLoss: %f\t\tTime cost: %1.f\t\tTrain Acc: %.2f%%\tValid Acc: %2.f%%\tLearning rate: %.6f' \
                %(epoch, tl, (time.time()-t), (tacc*100), (vacc*100), lr))
...
if __name__=='__main__':
    ...
    graph = tf.Graph()
    with graph.as_default():
        tf_data = {'train_X': tf.placeholder(tf.float32, shape=(batch_size, image_size, image_size, num_channels), name='x-train'),
                   'train_y': tf.placeholder(tf.float32, shape=(batch_size, num_labels), name='y-train'),
                   'valid_X': tf.constant(valid_dataset, name='x-valid'), 'valid_y': tf.constant(valid_labels, name='y-valid'),
                   'test_X' : tf.constant(test_dataset, name='x-test'),  'test_y' : tf.constant(test_labels, name='y-test')}
